...
class NaverApI:
    ...
    def get_request_url(self, url):
        req = Request(url)
        # Naver API 개인별 인증
        req.add_header('X-Naver-Client-Id', 'hmess2EC_4fFEYDZ3brr')
        req.add_header('X-Naver-Client-Secret', 'AZ6NdUjzkR')
...
def get_naver_search(self, node, search, start, display):
    ...
    retDate = self.get_request_url(url)
...
def get_post_data(self, post, outputs):
    ...
    # 'Tue, 09, mar 2023 17:04:00 +0900' 문자열로 들어온걸 날짜형으로 변경
